...
### Email quality follow-up — 2026-09-24

- Tightened application confirmation and status messages; the copy now consistently names the candidate and role, uses the 2–3-business-day review target on receipt, and explains that role assessments are only available after advancement and the technical check.
- HR application notices now carry only the candidate name, role, and application ID with a direct admin-record link. Candidate status emails no longer receive admin-only notes. Technical-check notices link to the matching application without putting the report or candidate email address in the message.
- Fixed dark-mode text classes in referral messages, added Outlook dark-mode treatments for callouts/status/button surfaces, and set inline logo visibility defaults for clients that ignore embedded styles. HTML-to-text conversion now keeps the destination URL for linked buttons.
- Removed the stale public “skills-check” claim from the landing-page process copy.
- Added template-level tests for all nine email renderers plus shared-shell, escape, link, status, device-label, and privacy cases; added these with role-question, recipient-parser, and assessment-gate tests to CI. The combined local suite passed 20/20, both app typechecks and the frontend production build passed, and Wrangler dry-run passed.
- These are source/local verification results only. No real test email was sent; actual Gmail, Apple Mail, and Outlook light/dark rendering and delivery remain unverified until there is a designated test inbox and client-side review.
- A local read-only hidden-job catalogue check was attempted but did not connect; it made no database changes. Hidden-role content is therefore still not reviewed.
