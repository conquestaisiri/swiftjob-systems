...
try {
  ...
  if (response.status !== 200 || body.code !== code || !Number.isInteger(body.referralRewardCents)) {
    throw new Error(`Unexpected public referral response (${response.status})`);
  }
  console.log(JSON.stringify({ status: "PASS", publicReferral: response.status, rewardCents: body.referralRewardCents }));
} finally {
  if (!keep) {
    await sql`DELETE FROM candidate_referrals WHERE link_code = ${code}`;
    await sql`DELETE FROM candidate_referral_links WHERE code = ${code}`;
