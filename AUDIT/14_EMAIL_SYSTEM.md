...
The controlled probes are recorded in `evidence/email-dns-closure.json`, `evidence/live-email-flow-closure.json` and `evidence/live-application-email-closure.json`. The production magic-link handler returned 200, Resend reported the message `delivered`, the token was verified successfully and the test session was revoked. A controlled production application then returned 201, sent HR notification and applicant confirmation messages, an authenticated admin transition returned 200 and sent the Reviewing status message, and all three Resend messages reported `delivered`. Provider payloads for all three messages contained the absolute logo reference, a plain-text alternative and the configured SwiftJob Reply-To domain; the synthetic application and uploaded resume were deleted and a follow-up read returned 404. A real mailbox test must still cover inbox/spam placement, links and Reply-To rendering. The Resend API key, sender authentication and Cloudflare recipient routing are verified; mailbox inspection remains NOT VERIFIED.
