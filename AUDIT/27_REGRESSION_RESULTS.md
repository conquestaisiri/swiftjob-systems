...
| Real provider email receipt | PARTIAL / inbox unverified | Production magic-link handler returned 200, Resend reported `delivered`, token verification/logout passed, direct-recipient and inbound `hr@swiftjob.payservice.top` probes returned `delivered`, and Cloudflare recorded the inbound probe as `Forwarded`. Inbox/spam placement, Reply-To inspection and application confirmation/status receipts still require owner verification |
...
| Production deploy of this branch | PASS (boundary) | Worker active version `002c32fd-b0c6-4f8f-aaae-160e07253c18`; latest Pages `e30419db.swiftjob-systems.pages.dev`; live domain smoke checks |
