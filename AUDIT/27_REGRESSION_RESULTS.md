...
| Real provider email receipt | PARTIAL / inbox unverified | Direct-recipient and inbound `hr@swiftjob.payservice.top` Resend probes returned `delivered`; Cloudflare Activity Log recorded the inbound probe as `Forwarded` after the `swiftjob` subdomain was enabled. Inbox/spam placement, Reply-To inspection and application-flow receipts still require owner verification |
...
| Hosted CI for this branch | PASS | GitHub Actions run `34681345770`; workspace typecheck, isolated workflow regressions and production frontend build passed. Production deploy jobs are intentionally gated to `main`. |
