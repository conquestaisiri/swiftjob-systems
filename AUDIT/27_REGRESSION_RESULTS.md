...
| Real provider email receipt | PARTIAL / inbox unverified | Controlled direct-recipient Resend probe returned `delivered` (`evidence/email-dns-closure.json`); five historical bounces target the configured `swiftjob.payservice.top` HR/Reply-To domain, which has no MX; inbox/spam, Reply-To and application-flow receipts still require owner verification |
