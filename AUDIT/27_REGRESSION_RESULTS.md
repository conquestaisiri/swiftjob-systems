...
| Real provider email receipt | PARTIAL / inbox unverified | Production magic-link handler returned 200, Resend reported `delivered`, token verification/logout passed, direct-recipient and inbound `hr@swiftjob.payservice.top` probes returned `delivered`, and Cloudflare recorded the inbound probe as `Forwarded`. Controlled production contact, application, admin custom-mail, referral-invitation and referral-click flows also returned successful handlers and Resend `delivered` events; synthetic application/resume/referrals were deleted. Inbox/spam placement, Reply-To rendering and link activation still require owner verification |
...
| Hosted CI for this branch | PASS | GitHub Actions run `34703400433`; workspace typecheck, isolated workflow regressions and production frontend build passed. Production deploy jobs are intentionally gated to `main`. |
