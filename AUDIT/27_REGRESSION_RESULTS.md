...
| Real provider email receipt | PARTIAL / inbox unverified | Production magic-link handler returned 200, Resend reported `delivered`, token verification/logout passed, direct-recipient and inbound `hr@swiftjob.payservice.top` probes returned `delivered`, and Cloudflare recorded the inbound probe as `Forwarded`. A controlled production application, HR notification, applicant confirmation and Reviewing status update also returned successful handlers and Resend `delivered` events; the synthetic record and resume were deleted. Inbox/spam placement, Reply-To rendering and link activation still require owner verification |
