...
| Real provider email receipt | PARTIAL / inbox unverified | Production magic-link handler returned 200, Resend reported `delivered`, token verification/logout passed, direct-recipient and inbound `hr@swiftjob.payservice.top` probes returned `delivered`, and Cloudflare recorded the inbound probe as `Forwarded`. A controlled production contact request and application, HR notification, applicant confirmation and Reviewing status update also returned successful handlers and Resend `delivered` events; the synthetic record and resume were deleted. Inbox/spam placement, Reply-To rendering and link activation still require owner verification |
...
| Hosted CI for this branch | PASS | GitHub Actions run `34701538185`; workspace typecheck, isolated workflow regressions and production frontend build passed. Production deploy jobs are intentionally gated to `main`. |
...
The runtime checks use synthetic people, database, R2, and email recipients. They prove application behavior in isolation and do not prove production delivery. Live checks prove the deployed public boundary, headers, protected-route responses, dynamic sitemap, proxy behavior and the active combined edge rule; they do not replace a real inbox receipt, DNS authentication check, historical credential rotation or fine-grained per-route edge policies.
