...
Remaining access needs are owner-side historical Supabase credential rotation and a token/plan with zone Rulesets/Rate Limiting permissions for programmatic or finer-grained edge-rule changes. Email Routing for the new domain is verified in `evidence/email-routing-live-closure.json`; the historical Supabase project token is valid and identified, but its old pooler credential still requires owner-side rotation/revocation.
